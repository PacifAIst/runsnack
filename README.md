...
🔒 [Security model & threat model](SECURITY.md) — Quick tip: power off for ~10 seconds between sessions (with different people only), so the GPU's memory is cleared.
...
That's the whole flow. First run pulls a ~1.4 GB image (a few seconds); every session after that starts in seconds.
...
**Hardware detection:** `snackup.sh` checks the host architecture and picks the right image automatically — the regular x86_64 image, a separate NVIDIA Jetson build (JetPack 6.1 only, detected via the standard `/etc/nv_tegra_release` marker), or a separate DGX Spark build (Grace Blackwell, detected via the DGX OS release file, DMI product name, or `nvidia-smi`'s reported GPU name) — or exits with a clear message on anything else, rather than silently pulling an image that won't work. This is intentional: Docker's own multi-arch resolution can't tell a Jetson or a DGX Spark apart from any other ARM64 machine, so the detection lives here instead. DGX Spark's image uses the same standard `--gpus all` mechanism as x86; only Jetson needs the special `--runtime nvidia` handling.
...
- **Runs on anything with Docker** — an NVIDIA Jetson (JetPack 6.1), a DGX Spark, an old gaming rig, a datacenter card, even CPU-only. Not limited to whatever a marketplace happens to stock.
...
| **Desktop ARM AI systems (e.g. DGX Spark)** | ✅ Yes | ⚠️ Rare / none | ❌ No | ❌ No | ❌ No | ❌ No |
